			...
			struct ramzswap_ioctl_stats *s)
{
	...
	unsigned int comp_ratio[5] = {0,};	// cyoung : pages with compression ratio
	...
	if (succ_writes && rs->pages_stored) {
		...
		no_compress_perc = rs->pages_expand * 100
					/ rs->pages_stored;
		// cyoung : pages with compression ratio
		comp_ratio[0] = rs->comp_ratio20;
		comp_ratio[1] = rs->comp_ratio40;
		comp_ratio[2] = rs->comp_ratio60;
		comp_ratio[3] = rs->comp_ratio80;
		comp_ratio[4] = rs->comp_ratio100;
	}
	...
	// cyoung : pages with compression ratio
	s->pages_comp20 = comp_ratio[0];
	s->pages_comp40 = comp_ratio[1];
	s->pages_comp60 = comp_ratio[2];
	s->pages_comp80 = comp_ratio[3];
	s->pages_comp100 = comp_ratio[4];
	}
...
//	float ratio = 0; // cyoung : compression ratio
	...
	// cyoung : pages with compression ratio
/*	ratio = (PAGE_SIZE - clen) * 100 / PAGE_SIZE;
	if (ratio < 20) {
		stat_dec(&rzs->stats.comp_ratio20);
	}
	else if (ratio < 40) {
		stat_dec(&rzs->stats.comp_ratio40);
	}
	else if (ratio < 60) {
		stat_dec(&rzs->stats.comp_ratio60);
	}
	else if (ratio < 80) {
		stat_dec(&rzs->stats.comp_ratio80);
	}
	else {
		stat_dec(&rzs->stats.comp_ratio100);
	}
*/	
out:
...
static int ramzswap_read(struct ramzswap *rzs, struct bio *bio)
{
	...
	printk("ramzswap_read() : page check\n");
	/* Requested page is not present in compressed area */
	if (!rzs->table[index].page)
		return handle_ramzswap_fault(rzs, bio);

	printk("ramzswap_read() : Uncompressed\n");
	...
	printk("ramzswap_read() : decompress\n");
	...
	printk("ramzswap_read() : flush\n");
	ramzswap_flush_dcache_page(page);

	set_bit(BIO_UPTODATE, &bio->bi_flags);
	bio_endio(bio, 0);

	printk("ramzswap_read() : end\n");
	return 0;
...
memstore:
	...
	ratio = (PAGE_SIZE - clen) * 100 / PAGE_SIZE;
	if (ratio < 20) {
		stat_inc(&rzs->stats.comp_ratio20);
	}
	else if (ratio < 40) {
		stat_inc(&rzs->stats.comp_ratio40);
	}
	else if (ratio < 60) {
		stat_inc(&rzs->stats.comp_ratio60);
	}
	else if (ratio < 80) {
		stat_inc(&rzs->stats.comp_ratio80);
	}
	else {
		stat_inc(&rzs->stats.comp_ratio100);
	}
