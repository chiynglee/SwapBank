...
/*
 * Max compressed page size when a user wants to compress all pages
 * If the compressed page size is bigger than this value, 
 * the page is not compressed
 */
static const unsigned max_zpage_size_all = PAGE_SIZE;
...
int ramzswap_init(unsigned int num_devs, unsigned long size_kb, unsigned long limit_kb, char *backing);
void ramzswap_exit(void);
