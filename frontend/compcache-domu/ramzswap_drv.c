...
//		max_zpage_size = max_zpage_size_all;
...
int ramzswap_init(unsigned int num_devs, unsigned long size_kb, unsigned long limit_kb, char *backing)
{
	...
	disksize_kb = size_kb;
	memlimit_kb = limit_kb;
	strncpy(backing_swap, backing, MAX_SWAP_NAME_LEN);
...
//void __exit ramzswap_exit(void)
